...
> 当前版本：**v3.2.6** —— 可视化管理后台，保存即提交 GitHub，Cloudflare 自动部署。
...
顶栏右侧提供 **？操作说明** 与 **ⓘ 关于** 入口；移动端自动收起为抽屉式导航。
...
- 点击先弹出**风险确认框**（提示将直接以源码形式编辑、绕过表单校验、异常需自行承担）；点「不同意」关闭弹窗、留在可视化，点「同意并继续」切换为源码编辑器（可改可存）。
